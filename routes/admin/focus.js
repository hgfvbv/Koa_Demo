...
router.get('/', async (ctx) => {
    ...
    let totalCount = await DB.getTotalCount('focus', {});
    let list = await DB.find('focus', {}, { 'sort': sortJson }, { pageIndex, pageSize });
    await ctx.render('admin/focus/index', { list, sortType, totalPage: Math.ceil(totalCount / pageSize), pageIndex });
}).get('/add', async (ctx) => {
    await ctx.render('admin/focus/add');
}).post('/doAdd', tools.multerInit('public/upload/focus').single('pic'), async (ctx) => {
    ...
    if (result.insertedCount > 0) {
        ctx.redirect(`${ctx.state.__ROOT__}/admin/focus`);
    } else {
        await ctx.render('admin/error', {
            msg: '对不起！服务器异常！请稍后再试！',
            redirect: `${ctx.state.__ROOT__}/admin/focus/add`
        });
    }
}).get('/edit', async (ctx) => {
    let _id = DB.getObjectId(ctx.query.id);
    let list = (await DB.find('focus', { _id }))[0];
    await ctx.render('admin/focus/edit', {
        list,
        prevPage: ctx.state.G.prevPage   /*保存上一页的值*/
    });
}).post('/doEdit', tools.multerInit('public/upload/focus').single('pic'), async (ctx) => {
    //注意：一定要加上 upload.single('img_url')  否则取不到值
    //multer图片上传插件重置了post的取值方式，bodyparser失效，
    //需用multer指定的ctx.req.body取值，ctx.req.file.path获取上传好的图片路径
    ...
    if (result.modifiedCount > 0) {
        ...
        await ctx.render('admin/error', {
            msg: '对不起！服务器异常！请稍后再试！',
            redirect: `${ctx.state.__ROOT__}/admin/focus/edit?id=${id}`
        });
    }
});
